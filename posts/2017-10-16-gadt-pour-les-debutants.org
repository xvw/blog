...
*Gabriel Scherer[fn:13]* pour m'avoir aidé dans l'a rédaction de cet article, 
respectivement pour m'avoir expliqué des traits relatifs à *Scala[fn:14]* 
et pour m'avoir donné plus d'éléments théoriques sur ce que sont réellement les GADTs et 
les travers liés à l'anaologie entre GADTs et héritage.
#+END_QUOTE
...
Sans rentrer dans les détails de l'implémentation, on défini deux *cases*, un objet, qui n'aura
...
que src_scala{head} est une méthode de notre classe src_scala{Some} :
...
Concernant les GADTs, Nicolas Rinaudo m'a transmis un article[fn:15] qui explique pourquoi, en jouant 
...
L'article[fn:15] propose d'ajouter des 
...
Cependant, *Dotty[fn:16]* (un compilateur expérimental pour le langage Scala, qui fait aussi 
d'incubateur et qui a pour vocation de faire reposer Scala sur des bases plus formalisées) 
...
des cas d'usages où les GADTs sont essentiel, surtout si comme moi, vous débutez avec 
les langages fonctionnels statiquement typés. J'espère tout de même que cet article 
...
et tous les membres de la communauté *LilleFP* pour m'avoir forcé (voir oppressé) pour la 
...
[fn:15] https://pchiusano.github.io/2014-05-20/scala-gadts.html
[fn:16] http://dotty.epfl.ch/
